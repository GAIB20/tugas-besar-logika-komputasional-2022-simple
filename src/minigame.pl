...
minigame :- write('Hallo ANDA Sudah di MINIGAME INPUTKAN TEBAKAN anda (range 1-10), 1 koin = 1000'),retract(countguess(A)),C is A+1,assertz(countguess(C)),
...
writehasilminigame :- curPlayer(P),youget(F),write('You get '),write(F),write(' koin'),getmoneypemain(P,S),X is S+F*1000,ubahMoney(X),
