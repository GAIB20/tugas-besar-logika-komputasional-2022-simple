...
checkRoll(X,Y) :- X > 0, X =< 20, Y = 1, print('\nAnda mendapat Tax Card. Anda dipinidahkan ke petak pajak.\n').
checkRoll(X,Y) :- X > 20, X =< 45, Y = 2, print('\nAnda mendapat Gift Card. Anda mendapat hadiah uang sejumlah 10000.\n').
checkRoll(X,Y) :- X > 45, X =< 65, Y = 3, print('\nAnda mendapat Go To Jail Card. Anda dipindahkan ke petak penjara.\n').
checkRoll(X,Y) :- X > 65, X =< 90, Y = 4, print('\nAnda mendapat Get Out Of Jail Card. Simpan kartu untuk digunakan.\n').
checkRoll(X,Y) :- X > 90, X =< 100, Y = 5, print('\nAnda mendapat Angel Card. Simpan kartu untuk digunakan.\n').
